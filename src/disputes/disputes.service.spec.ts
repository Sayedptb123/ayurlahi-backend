...
describe('DisputesService.findAll — SEC-7 org-scoping', () => {
  it('admin/support role sees everything, unfiltered (unchanged)', async () => {
    ...
    const result = await service.findAll('u1', 'SUPER_ADMIN', {} as any, undefined);
    expect(qb.andWhere).not.toHaveBeenCalled();
    expect(result.data).toHaveLength(1);
  });

  it('non-admin caller with a resolved organisationId is scoped to it (unchanged)', async () => {
    const { service, qb } = makeService([]);
    await service.findAll('u1', 'OWNER', {} as any, 'org-1');
  ...
  it('non-admin caller with NO resolved organisationId gets nothing, not everything', async () => {
    ...
    const result = await service.findAll('u1', 'OWNER', {} as any, undefined);
